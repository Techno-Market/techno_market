        ...
        <ul class="product-box flex fww">
            ...
            <li>
                <a href="/sales_post/detail/{article.id}">
                    <!-- {#each article.photo as photo} -->
                    {#if article.photo && article.photo[0]}
                    <div class="img-box rel">
                        <img src={`http://localhost:8080/gen/${article.photo[0].filePath}`} alt={article.photo[0].origFileName}>
                    ...
                    <div class="mt12 flex aic jcsb">
                        <h4 class="c222 f18 tb">{article.price}</h4>
