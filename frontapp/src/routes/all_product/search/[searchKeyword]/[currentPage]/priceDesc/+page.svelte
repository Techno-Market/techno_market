	...
	function generatePageButtons(totalPages) {
        ...
        async function changePage(searchKeyword ,page) {
            try {
                ...
                const response = await axios.get(`http://localhost:8080/api/articles/priceDesc?kw=${searchKeyword}&page=${page + 1}`);
                data.result = response.data; // 데이터 업데이트
                
                // 페이지 이동 시 브라우저의 주소도 업데이트
                goto(`/all_product/search/${searchKeyword}/${page}/priceDesc`);
            } catch (error) {
...
<div class="sub-cnt-area w100per rel zi1">
    <div class="con w100per">
		<h1 class="title-text lh120 tb">'{data.searchKeyword}' 검색 결과 - {data.result.data.articles.totalElements}개의 상품</h1>
		{#if data.result.data.articles.content.length === 0}
			<h2 class="title-text lh120 tb">검색 결과가 없습니다..</h2>
		{/if}
		<a href="/all_product/search/{data.searchKeyword}/0/priceDesc">가격 높은 순</a>
		<a href="/all_product/search/{data.searchKeyword}/0/priceAsc">가격 낮은 순</a>
...
<div class="paging-box flex jcc mt40">
	<ul class="flex aic jcc">
		{#if data.result.data.articles.number > 0}
			...
			<li class="page-btn" on:click={() => changePage(data.searchKeyword, data.result.data.articles.number - 1)}>
				<a href="">이전</a>
			</li>
		{/if}
		...
		<!-- svelte-ignore a11y-click-events-have-key-events -->
			...
			<li
				class="num"
				on:click={() => data.result.data.articles.number !== button - 1 && changePage(data.searchKeyword, button - 1)}
			>
