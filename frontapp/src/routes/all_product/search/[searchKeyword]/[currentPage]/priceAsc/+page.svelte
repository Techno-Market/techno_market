	...
	function generatePageButtons(totalPages) {
        ...
        async function changePage(searchKeyword ,page) {
            try {
                ...
                const response = await axios.get(`http://localhost:8080/api/articles/priceAsc?kw=${searchKeyword}&page=${page + 1}`);
                data.result = response.data; // 데이터 업데이트
                
                // 페이지 이동 시 브라우저의 주소도 업데이트
                goto(`/all_product/search/${searchKeyword}/${page}/priceAsc`);
            } catch (error) {
